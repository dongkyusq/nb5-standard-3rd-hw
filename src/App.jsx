...
function App() {
  ...
  const onChange = (event) => setMinAge(event.target.value);
  ...
  return (
    <div>
      ...
      살 이상
      <ul>
        {/* TODO: map을 사용하여 필터링된 학생들의 정보를 표시하세요. ex) 홍길동 - Age: 24, Grade: A */}
        {filteredStudents.map((students, index) => (
          <li key={index}>
            {students.name} - Age: {students.age}, Grade: {students.grade}
          </li>
        ))}
      </ul>
    </div>
